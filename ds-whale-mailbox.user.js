...
(function () {
  ...
  function addCustomChar(name, emoji, prompt) {
    charSeq++;
    const id = 'custom_' + charSeq + '_' + Math.random().toString(36).slice(2, 6);
    const ch = { id: id, name: name || '新角色', emoji: emoji || '🐠', tagline: '自定义', prompt: prompt || '', gradient: null };
  ...
  function scheduleEmailFromAI(userText) {
    ...
    setTimeout(function () {
      ...
      const pool = (ch.emails[category] && ch.emails[category].length) ? ch.emails[category] : ch.emails.generic;
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '#dsm-root{--dsm-glass:rgba(255,255,255,.6);--dsm-text:#20242e;--dsm-sub:#767d92;--dsm-line:rgba(255,255,255,.75);--dsm-accent:#6366f1;--dsm-accent2:#22d3ee;--dsm-bubble-user:linear-gradient(135deg,#34d399,#22d3ee);--dsm-bubble-user-text:#ffffff;--dsm-bubble-char:rgba(255,255,255,.78);--dsm-bubble-char-text:#20242e;--dsm-shadow:0 14px 44px rgba(31,38,70,.2);--dsm-avatar-user:linear-gradient(135deg,#a5b4fc,#818cf8);}',
      '#dsm-root.dsm-dark{--dsm-glass:rgba(30,33,42,.62);--dsm-text:#eef0f6;--dsm-sub:#9aa1b5;--dsm-line:rgba(255,255,255,.13);--dsm-bubble-char:rgba(255,255,255,.1);--dsm-bubble-char-text:#eef0f6;--dsm-shadow:0 14px 44px rgba(0,0,0,.55);}',
      ...
      '.dsm-badge{position:absolute;top:-3px;right:-3px;min-width:19px;height:19px;padding:0 5px;border-radius:10px;background:linear-gradient(135deg,#f43f5e,#fb7185);color:#fff;font-size:11px;font-weight:700;display:none;align-items:center;justify-content:center;box-shadow:0 0 0 2px rgba(255,255,255,.95),0 4px 10px rgba(244,63,94,.45);}',
      ...
      '.dsm-panel-head{position:relative;z-index:7;display:flex;align-items:center;gap:10px;padding:14px 16px 10px;}',
      ...
      '.dsm-contacts{position:relative;z-index:1;display:flex;gap:8px;padding:4px 14px 12px;overflow-x:auto;scrollbar-width:thin;}',
      ...
      '.dsm-chat{position:relative;z-index:1;display:flex;flex-direction:column;flex:1;min-height:0;margin:0 10px 12px;background:rgba(255,255,255,.18);border:1px solid var(--dsm-line);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);}',
      ...
      '.dsm-input-row{display:flex;gap:8px;align-items:flex-end;padding:10px 14px 22px;border-top:1px solid var(--dsm-line);border-radius:0 0 18px 18px;background:rgba(255,255,255,.12);}',
      '.dsm-dark .dsm-input-row{background:rgba(255,255,255,.03);}',
      ...
      '.dsm-set-sec{font-size:12px;font-weight:700;color:var(--dsm-sub);margin-top:4px;border-top:1px solid var(--dsm-line);padding-top:10px;}',
  ...
  function buildPanel() {
    ...
    panelEl.appendChild(contactsEl);
  ...
  function buildSettings(settingsBtn) {
    ...
    titleRow.appendChild(el('div', 'dsm-set-title', '⚙️ 智能回复设置'));
    ...
    const secTitle = el('div', 'dsm-set-sec', '🎭 角色人设（性格 / 背景提示词）');
    ...
    function addNewRow() {
      ...
      row.appendChild(ta);
      charsWrap.appendChild(row);
      const rec = { nameIn: nameIn, emojiIn: emojiIn, ta: ta };
      newRows.push(rec);
      ...
      function commit() {
        ...
        const ch = addCustomChar(name, String(emojiIn.value || '').trim() || '🐠', String(ta.value || '').trim());
    ...
    saveBtn.addEventListener('click', function () {
      ...
      newRows.forEach(function (r) {
        ...
        if (!name) return;
        const ch = addCustomChar(name, String(r.emojiIn.value || '').trim() || '🐠', String(r.ta.value || '').trim());
        addedId = ch.id;
      });
      newRows.length = 0;
      saveSettings();
    ...
    settingsPane.appendChild(rowSmart);
    settingsPane.appendChild(rowKey);
    settingsPane.appendChild(rowModel);
    settingsPane.appendChild(btns);
    settingsPane.appendChild(status);
    settingsPane.appendChild(hint);
    settingsPane.appendChild(secTitle);
    settingsPane.appendChild(charsWrap);
    settingsPane.appendChild(addBtn);
  ...
  function callSmartReply(charId, text, apiKey, model) {
    ...
    const ch = getChar(charId) || CHARACTERS.sister;
    const messages = [
      { role: 'system', content: charPrompt(charId) },
